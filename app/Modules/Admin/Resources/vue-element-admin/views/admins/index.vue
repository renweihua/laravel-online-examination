<template>
    <div class="app-container">
        <div class="filter-container">
            ...
            <el-button v-if="permission.delete.status" v-waves class="filter-item" type="danger" icon="el-icon-delete" @click="handleDelete">
                {{ $t('table.batchDelete') }}
            </el-button>
            <el-button
        ...
        >
            ...
            <el-table-column label="创建时间" show-overflow-tooltip align="center">
                <template slot-scope="{ row }">
                    {{ row.admin_info ? (row.admin_info.created_time | parseTime("{y}-{m}-{d} {h}:{i}")) : "" }}
                </template>
            </el-table-column>
            ...
            >
                <template v-slot="{row}">
                    <!-- 状态变更 -->
                    <el-button v-if="permission.change_filed.status && row.admin_id > 1 && row.is_check == 0" type="text"
                    ...
                    <el-button v-else-if="permission.change_filed.status && row.admin_id > 1 && row.is_check == 1" type="text"
                    ...
                    <el-button v-if="permission.delete.status && row.admin_id != 1" type="text" icon="el-icon-delete" @click="handleDelete(row)">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    import {getList, create, update, setDel, changeFiledStatus} from '@/api/admins';
